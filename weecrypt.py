...
def other_nicks ( channel_name, server_name ):
    nicks = []
    infolist = weechat.infolist_get ( "irc_nick", "", server_name + "," + channel_name )
...
def decrypt ( message ):
    p = subprocess.Popen ( [ "base64", "-d", "-w", "0" ], stdin = subprocess.PIPE, stdout = subprocess.PIPE )
...
def out_modifier ( data, modifier, server_name, irc_message ):
    ...
    def build_message ( add_end = True ):
        message = "PRIVMSG " + parsed [ "channel" ] + " :" + new_message
        if add_end:
            message += "crypt"
        return message
